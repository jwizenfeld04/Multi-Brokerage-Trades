...
from .redbridge_apis.dspac_api_new import DSPACAPI
from .base_client import BaseClient


class DspacClient(BaseClient):
    def __init__(self):
        super().__init__(brokerage_name="DSPAC")
        load_dotenv()
        username = os.getenv("DSPAC_USERNAME")
        password = os.getenv("DSPAC_PASSWORD")
        self.dspac = DSPACAPI(
            username, password, filename="dspac_credentials.pkl", creds_path="./creds/"
        )
        self.authenticated = False
        self.accounts = {}

    async def authenticate(self) -> bool:
        """Asynchronous method to authenticate using DSPAC API."""
        try:
            # Run the blocking make_initial_request in a thread
            await asyncio.to_thread(self.dspac.make_initial_request)
            use_email = "@" in self.dspac.user
    ...
    async def _login(self, use_email: bool) -> bool:
        try:
            ticket_response = await asyncio.to_thread(
                self.dspac.generate_login_ticket_email
                if use_email
                else self.dspac.generate_login_ticket_sms
            )
            ...
            login_response = await asyncio.to_thread(
                self.dspac.login_with_ticket, ticket
            )
    ...
    async def _solve_captcha(self, use_email):
        try:
            captcha_image = await asyncio.to_thread(self.dspac.request_captcha)
            ...
            sms_request_response = await asyncio.to_thread(
                (
                    self.dspac.request_email_code
                    if use_email
                    else self.dspac.request_sms_code
                ),
    ...
    async def _get_captcha_input(self, captcha_image):
        file = BytesIO()
    ...
    async def _send_sms_code(self, use_email, captcha_input=None):
        sms_code_response = await asyncio.to_thread(
            self.dspac.request_email_code if use_email else self.dspac.request_sms_code,
            captcha_input=captcha_input,
        )
    ...
    async def get_account_balance(self, account_id: str) -> float:
        if not self.accounts:
            return None
        try:
            account_assets = await asyncio.to_thread(self.dspac.get_account_assets)
    ...
    async def get_stock_holdings(self, account_id: str) -> Dict[str, float]:
        if not self.accounts:
            ...
            return False
        try:
            positions = await asyncio.to_thread(self.dspac.get_account_holdings)
            stocks = {}
    ...
    async def get_stock_price(self, ticker: str) -> float:
        ...
        try:
            response = await asyncio.to_thread(
                self.dspac.validate_buy, ticker, 1, 1, self.accounts.get(1)
            )
    ...
    async def _place_buy_orders(
        ...
        try:
            response = await asyncio.to_thread(
                self.dspac.execute_buy, ticker, 1, account_id, False
            )
    ...
    async def _place_sell_orders(
        ...
        try:
            ...
            response = await asyncio.to_thread(
                self.dspac.execute_sell, ticker, 1, account_id, entrust_price, False
            )
            if response.get("Outcome") != "Success":
                print(response)
                raise Exception
